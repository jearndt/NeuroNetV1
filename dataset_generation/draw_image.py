...
def draw_line_with_angle(width, height, angle, line_length, thickness=2, img=None, center_x=None, center_y=None):
...
def create_dataset(output_dir, filtered_dir, width=25, height=25, line_length=15, thickness=2, positions=4, variations=20):
        ...
        for var in range(variations):
            if angle in [45, 135]:
                for i, (cx, cy) in enumerate(corners):
                    ...
                    img_name = f"angle_{angle}_position_{i}_var_{var}.png"
                    ...
                    filtered_img_name = f"filtered_angle_{angle}_position_{i}_var_{var}.png"
                ...
                for pos in range(positions):
                    ...
                    img_name = f"angle_{angle}_position_{pos}_var_{var}.png"
                    img_path = os.path.join(output_dir, img_name)
                    ...
                    # Apply Gaussian filter and save the result
                    filtered_img = apply_gaussian_filter(img)
                    filtered_img_name = f"filtered_angle_{angle}_position_{pos}_var_{var}.png"
                    filtered_img_path = os.path.join(filtered_dir, filtered_img_name)
                    
                    save_img(filtered_img_path, filtered_img)

if __name__ == '__main__':
    output_dir = 'data_images_new'
    filtered_dir = 'filtered_images_new'
